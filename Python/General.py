...
elif        # else if; in an if block, conditionally trigger this when if is false
...
from        # from <module> import <object> [as <name>] --bracketed code is optional
...
not         # opposite of 'is', check for absence of something, e.g. 'if not None' --> True
...
return      # return a value from a function, do not execute subsequent code in function
try         # in a try-except block, 'try' some code and handle some 'except'ions
with        # with(filename, mode) --> execute code block with 'filename' open then close 'filename'
...
# del
del k # deletes k from memory, will cause NameError if called without re-initializing!
...
try:
    print 5 + '5'
except TypeError, ValueError: # catch either of these errors, but cant save error msg to e
...
# pass --- aka, do nothing on the line where 'pass' is located (NOT the same as break!)
try:
    5 + '5'
except:
    pass
...
def ReturnExample(arg):
    if arg < 10:
        return arg * 10 # if triggered, 2nd return does not fire b/c function returns and stops execution
